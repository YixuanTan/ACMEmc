...
//    mat_para[i].grain_orientations = grain_orientations;
    ...
    double t_inc = (pow(pow(lambda, m) * K1 * (tmc_at_PdenominatorMax_global + 1) + pow(L_initial, m), (double)n/m) - pow(pow(lambda, m) * K1 * tmc_at_PdenominatorMax_global + pow(L_initial, m), (double)n/m))/K_/exp(-Q/R/tmp_at_PdenominatorMax_global);

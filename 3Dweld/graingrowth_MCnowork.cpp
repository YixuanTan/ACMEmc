    ...
    for(int codx=x0(*grid, 0); codx < x1(*grid, 0); codx++) 
      for(int cody=x0(*grid, 1); cody < x1(*grid, 1); cody++) 
        for(int codz=x0(*grid, 2); codz < x1(*grid, 2); codz++){
          ...
          coords[1] = cody;
          coords[2] = codz;
          (*grid)(coords) = coords[0]*dim_y*dim_z + coords[1]*dim_z + coords[2];
          (*grid).AccessToTmc(coords) = tmc_initial;
...
template <int dim> void* flip_index_helper( void* s )
{
  ...
  int rank = MPI::COMM_WORLD.Get_rank();
  srand((unsigned)time(NULL)+rank); /* seed random number generator */
  flip_index<dim>* ss = static_cast<flip_index<dim>*>(s);
  double kT=0.0;
	vector<int> x (dim,0);
  int first_cell_start_coordinates[dim];
...
/*
    ...
    double site_selection_probability = 1;//Pnumerator/ss->Pdenominator;
        ...
        for (int j=-1; j<=1; j++) {
		  ...
		  for (int i=-1; i<=1; i++){
            ...
            if(!(i==0 && j==0 && k==0)){
              ...
              if(r[0]<x0(*(ss->grid), 0)-1 || r[0]>x1(*(ss->grid), 0) || r[1]<x0(*(ss->grid), 1)-1 || r[1]>x1(*(ss->grid), 1) ||
                 r[2]<x0(*(ss->grid), 2)-1 || r[2]>x1(*(ss->grid), 2))
                continue;// neighbour outside the global boundary, skip it. 
				      unsigned long spin = (*(ss->grid))(r);
              neighbors.push_back(spin);
              if(spin==spin1) 

...
unsigned long generate(MMSP::grid<dim,unsigned long >*& grid, int seeds, int nthreads)
{
	...
	if (dim == 2) {
		...
		if (number_of_fields==0) number_of_fields = static_cast<int>(float(dim_x*dim_y)/(M_PI*4.0*4.0)); /* average grain is a disk of radius XXX
...
template <int dim> double PdenominatorMax(MMSP::grid<dim, unsigned long>& grid, double& tmc_at_PdenominatorMax){
...
//std::cout<<"site_temperature is "<<site_temperature<<std::endl;
    double t_inc = (pow(pow(lambda, m) * K1 * (tmc_at_PdenominatorMax_global + 1) + pow(L_initial, m), (double)n/m) - pow(pow(lambda, m) * K1 * tmc_at_PdenominatorMax_global + pow(L_initial, m), (double)n/m))/K_/exp(-Q/R/site_temperature);

...
template <int dim> void UpdateLocalTmp(MMSP::grid<dim, unsigned long>& grid, double physical_time){
     ...
     for(int codx=x0(grid, 0); codx < x1(grid, 0); codx++)  
         for(int cody=x0(grid, 1); cody < x1(grid, 1); cody++) 
           for(int codz=x0(grid, 2); codz < x1(grid, 2); codz++){
             ...
             coords[1] = cody;
             coords[2] = codz;
             double r = pow(pow(lambda*codx,2) + pow((lambda*cody-Tv*physical_time), 2) + pow(lambda*codz,2),0.5); // in mm
             grid.AccessToTmp(coords) = T0 + (double)(Tq*Teta/2/3.1415926/Tk/r*exp(-Tlambda*Tv*(r-Tv*physical_time)));
             if(grid.AccessToTmp(coords)>1878.0)      
               grid.AccessToTmp(coords) = 1878.0;
           }
   }
}
  ...
  if(dim==2){
    for(int codx=x0(*grid, 0); codx < x1(*grid, 0); codx++) 
      for(int cody=x0(*grid, 1); cody < x1(*grid, 1); cody++){
        ...
        (*grid).AccessToTmp(coords) = 1284.0;
...
unsigned long growthexperiment(MMSP::grid<dim,unsigned long >*& grid, const char* filename)
{
  ...
  int nx,ny,nz;
	...
	if (dim == 2) {
	  nx = 1000;
		ny = 1000;
		grid = new MMSP::grid<dim,unsigned long>(0, 0, nx, 0, ny);
		#ifdef MPI_VERSION
		MPI::COMM_WORLD.Barrier();
		#endif
	} else if (dim == 3) {
...
template <int dim> void* flip_index_helper( void* s )
{
  ...
  int cell_coords_selected[dim];
	for (int hh=0; hh<ss->num_of_points_to_flip; hh++) {
//std::cout<<"ss->num_of_points_to_flip "<<ss->num_of_points_to_flip<<std::endl;
//getchar();
	  // choose a random cell to flip
    int cell_numbering_in_thread = rand()%(ss->num_of_cells_in_thread); //choose a cell to flip, from 0 to num_of_cells_in_thread-1
    if(dim==2){
   ...
   if(dim==2){
       for(int codx=x0(grid, 0); codx < x1(grid, 0); codx++) 
         for(int cody=x0(grid, 1); cody < x1(grid, 1); cody++){
           ...
           grid.AccessToTmc(coords) = double(pow(1.0/K1/lambda*pow(exp_eqn_rhs+pow(L0, n),1.0/n)-1.0/K1, 1.0/n1));  
         }
   }
   else if(dim==3){
     for(int codx=x0(grid, 0); codx < x1(grid, 0); codx++)  
     ...
     for(int codx=x0(grid, 0); codx < x1(grid, 0); codx++)  
         for(int cody=x0(grid, 1); cody < x1(grid, 1); cody++) 
           for(int codz=x0(grid, 2); codz < x1(grid, 2); codz++){
             ...
             coords[1] = cody;
             coords[2] = codz;
             std::cout<<"tmp "<<grid.AccessToTmp(coords)<<std::endl;
           }
...
std::cout<<"tmp_at_PdenominatorMax: "<<tmp_at_PdenominatorMax<<std::endl;
    MPI::COMM_WORLD.Barrier();
...
std::cout<<"t_inc: "<<t_inc<<" "<<tmp_at_PdenominatorMax_global<<" "<<(pow(lambda*(1+K1*pow(tmc_at_PdenominatorMax_global+1, n1)), n) - 
                     pow(lambda*(1+K1*pow(tmc_at_PdenominatorMax_global, n1)), n) )<<std::endl;
		unsigned long start = rdtsc();
    int num_of_sublattices=0;
    if(dim==2) num_of_sublattices = 4; 
    else if(dim==3) num_of_sublattices = 8;

...
template <int dim> void* flip_index_helper( void* s )
{
  ...
  int cell_coords_selected[dim];
    ...
    double Pnumerator = exp(-Q/R/temperature)/pow( pow(L_initial/lambda, n1) + K1 * t_mcs, (double)n/n1 - 1.0) ; //exp(-Q/R/temperature)/pow(t_mcs,n1-1)/pow((1+K1*pow(t_mcs,n1)),n-1);
   ...
   if(dim==2){
       for(int codx=x0(grid, 0); codx < x1(grid, 0); codx++) 
         for(int cody=x0(grid, 1); cody < x1(grid, 1); cody++){
           ...
           double Pdenominator = exp(-Q/R/temperature)/pow( pow(L_initial/lambda, n1) + K1 * tmc_local, (double)n/n1 - 1.0); //exp(-Q/R/temperature)/pow(tmc_local,n1-1)/pow((1+K1*pow(tmc_local,n1)),n-1);
...
//    mat_para[i].grain_orientations = grain_orientations;
    ...
    double t_inc = (pow(pow(lambda, n1) * K1 * (tmc_at_PdenominatorMax_global + 1) + pow(L_initial, n1), (double)n/n1) - pow(pow(lambda, n1) * K1 * tmc_at_PdenominatorMax_global + pow(L_initial, n1), (double)n/n1)) / K_/exp(-Q/R/tmp_at_PdenominatorMax_global); //(pow(lambda*(1+K1*pow(tmc_at_PdenominatorMax_global+1, n1)), n) - 
